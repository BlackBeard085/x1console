...
# Create test_envi directory if it doesn't exist
mkdir -p ~/test_envi
...
display_all_stake_info() {
    ...
    for wallet in "${stake_wallets[@]}"; do
        if [[ -f "$wallet" ]]; then
            ...
            stake_info=$(solana stake-account "$address")
            ...
            if [[ $? -eq 0 ]]; then
                ...
                echo "Error retrieving stake account information for $wallet."
            fi
        fi
    done
...
add_new_stake_account() {
    ...
    if (( wallet_count >= 5 )); then
        echo "Maximum stake wallet count reached. Please merge existing stake accounts to create more."
        return
    fi
...
merge_stake() {
    ...
    if solana merge-stake "$merge_to_address" "$merging_address"; then
        ...
        echo "The $merging_name can now be repurposed as a new stake account. Please run 'create-stake-account'."
    else
...
create_stake_account() {
    echo -e "\n--- Available Accounts To Create New Stake ---"
    ...
    if [[ ${#available_accounts[@]} -eq 0 ]]; then
        echo "No available accounts to create new stake accounts."
        return
    fi

    # User selects from available accounts by number
    read -rp "Select an account number to create a new stake account from: " chosen_account
...
show_menu() {
    ...
    echo "5. Merge Stake"
    echo "6. Create New Stake Account"
    echo "7. Exit"
}
