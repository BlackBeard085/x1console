...
echo "exported PATH and added to bashrc"
...
# Change to the x1console directory
cd $HOME/x1console || error_exit "Failed to change to directory X1 Console."
# Check if wallets.json exists, if it does, skip SCRIPT 2
if [[ -f "wallets.json" ]]; then
    echo "wallets.json exists. Skipping wallets creation."
    exit 0
fi
